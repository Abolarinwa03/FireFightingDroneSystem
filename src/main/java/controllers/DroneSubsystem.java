...
import controllers.PerformanceMonitor;
...
public class DroneSubsystem {
    ...
    public void sendFireExtinguishedStatus(int zoneId) {
        ...
        // Track fire extinguished for performance metrics
        PerformanceMonitor.getInstance().monitorFireExtinguished(zoneId);
    }
    ...
    public void handleFireEvent(FireEvent event) {
        performStateTransition(() -> {
            ...
            // Track fire event for performance metrics
            PerformanceMonitor.getInstance().monitorFireEvent(event, this);
        });
    }
    ...
    public void dropAgent() {
        performStateTransition(() -> {
            ...
            // Track agent drop for performance metrics
            PerformanceMonitor.getInstance().monitorAgentDrop(this, currentEvent);
        });
    }
    ...
    public void taskCompleted() {
        // Store the current event before it gets cleared
        FireEvent completedEvent = this.currentEvent;

        performStateTransition(() -> {
            currentState.taskCompleted(this);
            // Track mission completion for performance metrics
            PerformanceMonitor.getInstance().monitorMissionComplete(this, completedEvent);
        });
    ...
    public void setState(DroneState state) {
        String oldState = this.currentState.getClass().getSimpleName();
        ...
        // Track state change for performance metrics
        String newState = this.currentState.getClass().getSimpleName();
        PerformanceMonitor.getInstance().monitorDroneState(this, oldState, newState);
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            // Add shutdown hook to generate final performance report
